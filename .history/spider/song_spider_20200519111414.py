    ...
    try:
        ...
        for i in soup.find_all('li',class_="songlist-item clearfix"):
            ...
            cursor.execute("insert into songlist_details \
                (list_name,list_user,list_count,list_collect,list_tage,sname,sauthor) values ('{list_name}','{list_user}','{list_count}','{list_collect}','{listtags}',' {sname}','{sauthor}')".format(list_name=list_name,list_user=list_user,list_count=list_count,list_collect=list_collect,listtags=listtags,sname=sname,sauthor=sauthor))
            conn.commit()
            print(data)
