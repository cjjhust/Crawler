...
# %%
chart2 = df.sort_values('收藏次数',ascending=False).drop_duplicates('list_name') plt.figure(figsize=(14,6))
plt.title('百度歌单收藏次数曲线')
plt.xlabel('排名')
plt.ylabel('收藏次数') plt.plot(range(len(chart2['收藏次数'])),chart2['收藏次数']) plt.show()