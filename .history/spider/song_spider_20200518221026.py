...
def get_song_list_info(): #songListUrl
    try:
        ...
        # 歌单标签
        soup.find('div',attrs={"class":"songlist-info-tag"}).find_all('a')
        #list_tags = soup.select("div.songlist-info-tag > a").get_text()
        #list_tags=[]
        #for i in soup.find_all('div.songlist-info-tag > a'):
            #print(i)
            #list_tags.append(i)
