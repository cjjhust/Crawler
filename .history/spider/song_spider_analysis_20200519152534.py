...
# %%
chart1 = df.sort_values('list_count',ascending=False).drop_duplicates('list_name') plt.figure(figsize=(14,6))
plt.title('百度歌单播放次数曲线')
plt.xlabel('排名')
plt.ylabel('播放次数') plt.plot(range(len(chart1['播放次数'])),chart1['播放次数']) plt.show()